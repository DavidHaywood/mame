...
DEFINE_DEVICE_TYPE(HYPERNEOGEO64_SPRITES, hyperneogeo64_sprites, "hng64_sprites", "SNK HyperNeoGeo 64 Sprites")


hyperneogeo64_sprites::hyperneogeo64_sprites(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock)
	: device_t(mconfig, HYPERNEOGEO64_SPRITES, tag, owner, clock)
{
}

void hyperneogeo64_sprites::device_start()
{
}

void hyperneogeo64_sprites::device_reset()
{
}
...
void hyperneogeo64_sprites::draw_sprites(screen_device &screen, bitmap_rgb32 &bitmap, const rectangle &cliprect, uint32_t* spriteram, uint32_t* spriteregs, gfxdecode_device* gfxdecode)
{
	gfx_element *gfx;
	uint32_t *source = spriteram;
	uint32_t *finish = spriteram + 0xc000/4;

	// global offsets in sprite regs
	int spriteoffsx = (spriteregs[1]>>0)&0xffff;
	int spriteoffsy = (spriteregs[1]>>16)&0xffff;
	...
	// spriteregs[2] could also play a part as it also flips between 0x00000000 and 0x000fffff at the same time
	...
	bool zsort = !(spriteregs[0] & 0x01000000);
	...
	for(auto it : m_spritelist)
	{
		...
		/* Calculate the zoom */
		{
			...
			zoom_factor = (spriteregs[0] & 0x08000000) ? 0x1000 : 0x100;
		...
		if (spriteregs[0] & 0x00800000) //bpp switch
		{
			gfx= gfxdecode->gfx(4);
		}
		else
